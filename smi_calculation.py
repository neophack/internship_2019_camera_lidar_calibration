...
class Camera:
    def __init__(self, data_path):
        ...
        self.local_lidar = Lidar(self, data_path)
    ...
    def translation_pts_to_cam_sys(self, rotated_points, cam_angles = None):
        if cam_angles is None:
            cam_coord = [rotated_points[0] + self.local_lidar.lidar_pos_in_cam_sys[0],
                         rotated_points[1] + self.local_lidar.lidar_pos_in_cam_sys[1],
                         rotated_points[2] + self.local_lidar.lidar_pos_in_cam_sys[2]]
        else:
...
class Lidar:
    def __init__(self, data_path):
        ...
        self.local_cam = Camera(self, data_path)
    ...
    def projection_pts_on_cam(self, csv_data, cam_point = None):
        ...
        translated_to_cam_pts = self.local_cam.translation_pts_to_cam_sys(rotated_to_cam_points, cam_point [3:] )
    ...
    def dfScatter(self, i, df, xcol ='x', ycol ='y', catcol ='color'):
        ...
        img = ImageOps.mirror(self.local_camera.image_files[i])
...
class SMI_calculations:
    def __init__(self, data_path):
        ...
        self.local_lidar = Lidar(self, data_path)
        self.local_camera = Camera(self, data_path)
    ...
    def SMI_point_calculations(self, ):
        ...
        cur_points = self.local_camera.camera_rotation_angles.append(self.local_lidar.lidar_pos_in_cam_sys)
    ...
    def calc_SMI(self, points, Lidar_file, image):
        ...
        for i in range(len(Lidar_file)):
            pixel = self.local_lidar.projection_pts_on_cam(Lidar_file[i][:3], points)
            if pixel is not None:
                # print ("pixel ",pixel[:2])
                list_ref.append(Lidar_file[i][3])
    ...
    def SMI_Visualization(self, SMI_point, color = "Height"):
        for i in range(len(self.local_lidar.lidar_data)):
            ...
            dataset = self.local_lidar.lidar_data[i]
            pixels = []
            heights = []
            distances = []

            for j in range(len(self.local_lidar.lidar_data[i])):
                pixel, height, distance  = self.local_lidar.projection_pts_on_cam(dataset[j][:3], SMI_point)
                if pixel is not None:
                    pixels.append(pixel)
            ...
            if color == "Height":
                df = pd.DataFrame(pixels, columns=['x', 'y'])
