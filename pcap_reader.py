...
class PointReader:
    ...
    def seq_processing(self, data, seq_offset, seq_index, first_timestamp, pcap_num):
        ...
        Args:
            ...
            pcap_num: number of pcap file to be processed

        Function description:
        unpack sequence data into data_dict_list for each laser_id block
        with {'X', 'Y', 'Z', 'D', 'azimuth', 'laser_id','first_timestamp', 'pcap_num'} info

        data_dict_list pushed into self.seq_df
    ...
    def get_all_points_from_pcap(self, pcap_index):
        ...
        Function description:
            ...
            Dependent functions:
                ...
                return:
                        dataframe with unpacked lidar data for one pcap file

                """

        if pcap_index != self.processed_pcap:
            self.processed_pcap = pcap_index
            self.full_dataframe = self.local_point_reader.read_pcap(pcap_index)
