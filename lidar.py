...
class PointProcessing:
    ...
    def get_all_points_by_timestamp(self, timestamp, pcap_index, read_next_pcap_file, azimuth_bin=100, azimuth=None):
        ...
        if pcap_index != self.processed_pcap:
            ...
            print(timestamp, self.full_dataframe['first_timestamp'][-1])

        if read_next_pcap_file is not False:
            pcap_number= int(pcap_index)+1
            self.processed_pcap = "00"+str(pcap_number)
            index_dataframe = self.local_point_reader.read_pcap(self.processed_pcap, azimuth_bin)
            self.full_dataframe = pd.concat([self.full_dataframe, index_dataframe], ignore_index=True)
            print(timestamp, self.full_dataframe['first_timestamp'][-1])
